...
export class AccountModal extends Component {
  ...
  getRoleList = async () => {
    const { data: { roleList }, code } = await imService.getRoleList()
    if (code === "0") {
      this.setState({
        roleList
      })
  ...
  addAccount = async (params) => {      // 新增和更新账户
    ...
    return (
      ...
      >
        ...
        >
          ...
          <Form.Item name='roleName' label="请选择角色"
            rules={[{ required: true, message: `请选择角色` }]}
          >
            {/* <Input.Group > */}
            <Select placeholder="请选择角色" style={{ width: '80%' }}>
              {
                roleList.map(item => <Option value={item.roleName} key={item.id}>{item.roleName}</Option>)
              }
            </Select>
            {/* </Input.Group> */}
          </Form.Item>
